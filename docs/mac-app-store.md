...
- CCTrans Cloud prefers signed StoreKit AppTransaction JWS on native macOS App
  Store builds and falls back to the local App Store receipt for TestFlight/macOS
  cases where `AppTransaction.shared` is unavailable. ASC rejects
  `com.apple.developer.devicecheck.appattest-environment` on macOS uploads, so do
  not add App Attest to the MAS entitlements. Local unsigned MAS-dev builds
  cannot impersonate an App Store-signed install by signing into an Apple ID; use
  `CCTRANS_DEV_TOKEN` for local QA.
